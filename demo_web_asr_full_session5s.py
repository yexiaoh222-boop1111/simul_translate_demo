# -*- coding: utf-8 -*-
"""
实时语音翻译 Demo · Web 版：中文语音输入 → 英文语音同传（边说边译）
====================================================================
在 test_demo_copy.py 基础上改为"常驻服务 + 浏览器界面 + 流式同传"：

    模型常驻    启动即在后台线程并行加载三模型并预热，之后常驻显存，
                每次调用零加载等待。
    流式同传    检测到说话立即"开会话"：之后每隔 STREAM_POLL_S 对已攒的
                音频做一次识别，在最后一个标点处切分——切点前的文字立刻
                翻译、合成、播放，切点之后的尾巴留到下一轮重新识别（不按
                时间硬切，避免腰斩；也不按字符串长度切增量，避免头部被
                ASR 改写时错位）。说完后做最终收尾，存档整句。
...
日志（单一全局日志 logs/service_log.txt，行首 tag 便于 grep）：
    [record_log]   每次会话的完整过程（开会话/增量识别/增量翻译/合成/收尾/存档）
    [model_jiazai] 模型加载与预热
    [service]      其余服务事件（启动/开麦停麦/丢弃噪声/异常）

运行：
    conda activate demo
    python demo_web.py                    # 就绪后自动开麦
    python demo_web.py --mic 1            # 指定麦克风
    python demo_web.py --mock             # 不加载模型，假数据联调界面/日志
...
STREAM_POLL_S = 5.0        # 流式同传：每隔多久对已攒音频做一次增量识别翻译
PUNCT_CUT = re.compile(r"[。！？；，、!?;,…]")   # 切分标点：到最后一个标点处切，其后留到下轮
...
def last_punct_end(text):
    """返回文本中最后一个切分标点之后的下标（切点）；无标点返回 None。"""
    m = None
    for m in PUNCT_CUT.finditer(text):
        pass
...
# ---------------- 同传会话（一次说话：边说边译） ----------------
class Session:
    """流式同传会话：vad_loop 负责开/关会话，worker 线程按 tick 增量
    识别→翻译→合成，说完（停顿/超长）后做最终收尾 + 存档。"""

    def __init__(self, engine, first_seg):
        self.engine = engine
        self.lock = threading.Lock()    # 保护 blocks：vad_loop 收块 vs worker 标点切分
        self.blocks = [first_seg]       # 本会话 16k 音频（开场段先放进来，不丢首音节）
        self.audio_parts = []           # 已切走（已翻译）的音频段，收尾时拼回整句存档
        self.zh = ""                    # 已切分提交的识别文本累积（日志/存档用全量）
    ...
    def cut_at(self, audio_cut):
        """切掉前 audio_cut 个采样（已翻译部分），返回切掉的音频；剩余尾巴留在 blocks 里。"""
        with self.lock:
            audio = np.concatenate(self.blocks) if self.blocks else np.empty(0, np.float32)
            audio_cut = max(0, min(len(audio), audio_cut))
            seg_audio = audio[:audio_cut]
            rest = audio[audio_cut:]
            self.blocks = [rest] if len(rest) else []
            return seg_audio
...
class Engine:
    def __init__(self):
        ...
        self._utt_q = queue.Queue()     # 流水线任务: "tick" / "final" / None
    ...
    # ---- 同传流水线（worker 线程调用） ----
    def stream_tick(self, sess):
        """5s 增量同传：识别已攒音频 → 在最后一个标点处切 → 切点前翻完，切点后的尾巴留下轮。
        不按时间硬切，也不按字符串长度切增量（头部被 ASR 改写会错位）；按标点切音频，
        切点前的内容翻完即从缓冲移除，尾巴下一轮重新识别——从根上消除头部改写。"""
        if sess.seconds() < 0.8:
            return
        audio = sess.audio()
        zh, cost = transcribe(self.asr, audio)
        sess.cost_asr += cost
        cut = last_punct_end(zh)
        if cut is None or cut == 0:
            return   # 还没有标点，不强行切（避免连读被时间腰斩）
        cut_text = zh[:cut]
        if not any(ch.isalnum() for ch in cut_text):
            return
        # 按文字切点比例切音频：标点之后的尾巴留在 blocks 里，下一轮重新识别
        dur = sess.seconds()
        audio_cut = int(len(audio) * cut / max(1, len(zh)))
        sess.audio_parts.append(sess.cut_at(audio_cut))
        sess.zh += cut_text
        plog("record_log", f'句#{self.n} | 语音 {dur:.1f}s | 标点切分 {cost:.2f}s: '
                           f'"{cut_text}" (切 {cut}/{len(zh)} 字, 剩 {max(0, len(audio) - audio_cut)} 采样)')
        en, cost_mt = translate(self.mt_model, self.mt_tok, cut_text)
        sess.cost_mt += cost_mt
        plog("record_log", f'句#{self.n} | 增量翻译 {cost_mt:.2f}s: "{en}"')
        self.speak_frags(sess, en)
        broadcast({"type": "sess_tick", "n": self.n, "zh": sess.zh,
                   "en": "".join(sess.en_parts)})

    def finalize(self, sess):
        """收尾：全量识别尾巴 → 翻完播完 → 存档 → 推送最终卡片。"""
        sess.absorb(self.recorder)
        zh_tail, cost = transcribe(self.asr, sess.audio())
        sess.cost_asr += cost
        tail = zh_tail.strip()
        if any(ch.isalnum() for ch in tail):   # 流式没覆盖到的尾巴
            plog("record_log", f'句#{self.n} | 收尾识别 {cost:.2f}s: +"{tail}"')
            en, cost_mt = translate(self.mt_model, self.mt_tok, tail)
            sess.cost_mt += cost_mt
            self.speak_frags(sess, en)
        sess.zh += tail
        self.sess = None

        if not any(ch.isalnum() for ch in sess.zh):
        ...
        in_audio = np.concatenate(sess.audio_parts) if sess.audio_parts else np.empty(0, np.float32)
        tail_audio = sess.audio()
        full_in = np.concatenate([in_audio, tail_audio]) if len(tail_audio) else in_audio
    ...
    def speak_frags(self, sess, en):
        """翻译片段 → 切段合成 → 送播放队列（streamer 已停则只存档不外放）。"""
    ...
    def start_capture(self):
        ...
        plog("service", f"麦克风采集开始 (设备={'默认' if CFG['device'] is None else CFG['device']}), "
                        f"同传轮询 {STREAM_POLL_S}s, 断句停顿 {CFG['tail_silence']}s, "
        ...
        while self.running:
            ...
            try:
                if self.sess is None:
                    ...
                    if (self.sess and not self.sess.closing and self._utt_q.empty()
                            and time.perf_counter() - self.sess.last_tick >= STREAM_POLL_S):
                        self.sess.last_tick = time.perf_counter()
                        self._utt_q.put("tick")
            except Exception as e:
...
HTML_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "demo_web.html")
...
class MockVad:
    """mock 断句：3s 开会话，3~6s 视为一直在说（tick 有增量），6s 后伪造静音收尾。"""
